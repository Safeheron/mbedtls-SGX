...
int c1_ssl_client_main(void)
{
    ...
    mbedtls_printf("  . pem: %s ...\n\n", trusted_cas_pem);
    mbedtls_printf("  . len: %d ...\n\n", trusted_cas_pem_len);

    ret = mbedtls_x509_crt_parse(&cacert, (const unsigned char *) trusted_cas_pem,
                                 sizeof trusted_cas_pem);
    if (ret < 0) {
    ...
    if ((ret = mbedtls_net_connect_ocall(&server_fd, SERVER_NAME,
